...
ctx.hitPlayer = (t, dmg, info) => {
  ...
  if (info.part === 'blade') {
    ...
    if (ret) {
      effects.tracer(info.point, player.eye, INK.RED, 0.03, 0.08); hud.tip('REFLECTED', 0.9); input.rumble(0.5, 0.4, 90);
...
function breakProp(br, dir, local, quiet = false) {
  ...
  if (br.kind === 'pinata') {
    ...
    if (game.mode === 'solo') game.addScore(25, 'PINATA');
...
const MODIFIERS = [
  ...
  { name: 'CAFFEINE · MOVE FASTER', apply: () => { enemies.mods.speed = 1.35; enemies.mods.damage = 0.85; } },
  { name: 'HEAVY INK · HIT HARDER', apply: () => { enemies.mods.speed = 0.9; enemies.mods.damage = 1.4; } },
  { name: 'SWARM · MORE, FRAGILE ENEMIES', apply: () => { enemies.mods.speed = 1.15; enemies.mods.damage = 0.9; } },
];
const tips = () => [
  `Hold <b>${hud.key('grapple')}</b> to reel · press again while swinging to release`,
  `Use <b>${hud.key('block')}</b> to block; some bullets reflect`,
  'Air kills score more · try not to touch down',
  `<b>${hud.key('grenade')}</b> throw grenade · pickups can refill grenades`,
  `Press <b>${hud.key('jump')}</b> again in the air for a double jump`,
];
const bossFor = (n) => BOSSES[(Math.floor(n / 5) - 1) % BOSSES.length];
const enemyName = (t) => ({ boss: 'DOODLER', eraser: 'ERASER', inkblot: 'INKBLOT' })[t] || t.toUpperCase();
function startWave(n) {
  ...
  if (boss) { hud.message('WAVE ' + n, enemyName(bossFor(n)) + ' APPROACHING', 3); audio.bossRoar(player.center); }
...
enemies.onKill = (e, info, over) => {
  ...
  if (info.source === 'katana') { label = over ? 'SEVERED' : 'SLASHED'; pts += 50; }
  ...
  else if (!player.body.onGround && info.source !== 'deflect') { label += ' · AIR KILL'; pts += 40; }
...
function endDash(blocked) { player.dashLock = false; game.focus.dash = null; player.body.vel.set(0, 0, 0); if (blocked) { player.weapons[player.katanaIndex].startSlash(player._weaponState(false, false, 0)); audio.katanaSwing(); hud.tip('BLOCKED · DASH MISSED', 1.2); } }
...
function respawnLocal() {
  player.reset(arenaSpawn()); player.name = myName; player.lastHitBy = null; player.lastHit = null; game.state = 'play'; player.shieldT = 2; hud.tip('RESPAWN SHIELD · 2s', 1.6);
...
function boardHTML(title = 'FREE-FOR-ALL') {
  ...
  return `<h3>${title}</h3>${rows.map(([id, s]) => `<div class="${id === net.id ? 'me' : ''}"><span>${s.name}${id === net.id ? ' (YOU)' : ''}</span><span>${s.kills} KILLS · ${s.deaths} DEATHS</span></div>`).join('')}<div class="foot">FIRST TO ${FFA_TARGET} KILLS · ${mmss(matchLeft)} REMAINING · LOBBY ${String(net.aliasCode || net.code || '').replace(/-\d+$/, '')}</div>`;
}
function checkWin() {
...
function endMatch(winner) {
  ...
  hud.setGameplayVisible(false); hud.showScreen(`<h1>${title}</h1><div class="scoreboard">${sortedScores().map(([id, s]) => `<div class="${id === net.id ? 'me' : ''}"><span>${s.name}</span><span>${s.kills} KILLS · ${s.deaths} DEATHS</span></div>`).join('')}</div><div class="go" id="overGo">RETURN TO LOBBY…</div>`);
}
...
net.onDisconnect = () => { if (lobby.order && lobby.order.some((id) => id !== lobby.hostId)) migrateHost(); else leaveOnline('HOST LEFT THE LOBBY'); };
...
async function _migrateHost() {
  ...
  const roster = (lobby.order || []).filter((id) => id !== oldHost && lobby.players.has(id)); if (!roster.length || !base) { leaveOnline('HOST LEFT THE LOBBY'); return; }
  ...
  hud.message('HOST LEFT', successor === myId ? 'YOU ARE THE NEW HOST' : 'MOVING TO A NEW HOST…', 2.6);
  if (successor === myId) {
    let ok = false;
    for (let tries = 0; tries < 2 && !ok; tries++) { try { await net.host({ isPublic: lobby.isPublic, code }); ok = true; } catch (e) { await sleep(800); } }
    if (!ok) { leaveOnline('COULD NOT TAKE OVER LOBBY'); return; }
    ...
    net.onAlias = () => { broadcastLobby(); hud.kill('LOBBY CODE ' + base + ' RESTORED', 0); }; net.claimAlias(base);
...
net.on('pdead', (d, from) => {
  ...
  else hud.kill(kn ? kn + ' ERASED ' + vn + how : vn + ' OFF THE PAGE', 0);
...
net.on('parry', (d) => { audio.shieldHit(player.center); input.rumble(0.35, 0.3, 60); effects.strokeBurst(player.eye.clone().addScaledVector(player.forward, 0.5), INK.ORANGE, 8, 5, { life: 0.2, size: 0.03 }); hud.kill(d.ret ? 'RETURNED TO SENDER' : 'DEFLECTED', d.ret ? 25 : 0); });
net.on('shots', (d, from) => {
...
net.on('cut', () => { if (player.grapple.state !== 'idle') { player.detachGrapple(false); effects.strokeBurst(player.center, INK.ORANGE, 8, 4, { life: 0.25, size: 0.03 }); hud.tip('YOUR ROPE WAS CUT', 1.3); input.rumble(0.5, 0.3, 80); } });
...
player.onFall = () => {
  ...
  hud.kill('OFF THE PAGE · KILL -1', 0);
...
function idleUpdate(dt) {
  ...
  if (idle > limit - IDLE_WARN && !idleWarned && canDrop) { idleWarned = true; hud.message('ARE YOU THERE?', 'MOVE OR YOU WILL BE REMOVED FOR IDLING', 3); audio.empty(); }
  if (idle <= limit - IDLE_WARN) idleWarned = false;
  if (idle > limit && canDrop) { const back = net.isHost ? null : String(net.aliasCode || net.code || '').replace(/-\d+$/, ''); leaveOnline(net.isHost ? 'LOBBY CLOSED: EVERYONE IDLE' : 'REMOVED FOR IDLING'); lobby.rejoinCode = back; if (back) showStart(); return; }
  // the host also clears out a client that has sat idle past the limit, in case its tab cannot do it itself
  if (net.isHost) for (const [id, r] of remote) if (r.idle && r.idleSince && performance.now() / 1000 - r.idleSince > limit - IDLE_FLAG + 15) { net.sendTo(id, 'kick', { reason: 'REMOVED FOR IDLING' }); const c = net.conns.get(id); setTimeout(() => { try { c && c.close(); } catch (e) { /* ignore */ } }, 500); }
}
net.on('kick', (d) => { const back = String(net.aliasCode || net.code || '').replace(/-\d+$/, ''); leaveOnline(d && d.reason || 'REMOVED FROM LOBBY'); lobby.rejoinCode = back; if (back) showStart(); });
let syncTick = 0;
function netUpdate(dt) {
  ...
  if (inMatch() && !game.over) { if (clockOn) matchLeft = Math.max(0, matchLeft - dt); if (net.isHost) { clockT -= dt; if (clockT <= 0) { clockT = 2; net.send('clock', { left: Math.round(matchLeft), on: clockOn }); } } hud.setTimer(clockOn ? mmss(matchLeft) : 'TIMER STARTS WHEN A PLAYER JOINS'); }
...
async function quickPlay() {
  ...
  setStatus('NO OPEN LOBBIES · CREATING A PUBLIC ONE…');
  await createLobby(true);
}
function friendlyError(err) {
  const m = String(err && err.message || err || ''); if (!m) return 'SOMETHING WENT WRONG';
  if (/networking library/.test(m)) return 'COULD NOT LOAD NETWORKING · CHECK YOUR CONNECTION AND REFRESH';
  if (/timed out|signalling/.test(m)) return 'COULD NOT CONNECT TO MATCHMAKING · CHECK YOUR CONNECTION';
  if (/no lobby with that code/.test(m)) return 'LOBBY CODE NOT FOUND · CHECK WITH YOUR FRIEND';
  if (/no answer/.test(m)) return 'LOBBY FOUND BUT CONNECTION FAILED · DIRECT CONNECTION MAY BE BLOCKED';
  if (/full/.test(m)) return 'LOBBY IS FULL · TRY ANOTHER CODE';
  if (/leave the lobby/.test(m)) return 'LEAVE YOUR CURRENT LOBBY FIRST';
  return m;
}
...
function onlineHTML() {
  return `<h1>PLAY ONLINE</h1><h2>Free-for-all · first to ${FFA_TARGET} kills · up to 10 players</h2>
    <div class="online" id="online">
      ...
      <div class="row"><button type="button" class="big" id="quickBtn">QUICK PLAY</button><span class="hint">Join an open public lobby, or open one for you</span></div>
      ...
      <div class="row"><span>Have a lobby code?</span><input type="text" id="codeBox" placeholder="CODE" maxlength="5" autocomplete="off"><button type="button" id="joinBtn">JOIN</button></div>
...
function lobbyHTML() {
  ...
  return `<h1>LOBBY</h1><h2>Free-for-all · first to ${FFA_TARGET} kills · ${n}/${net.maxPlayers} players</h2>
    <div class="online" id="online">
      ...
      <div class="hint">${lobby.isPublic ? 'Public lobby: anyone can join with Quick Play or the code' : 'Private lobby: friends can join with this code'}</div>
      ...
      <div class="status" id="status">${esc(lobby.status || '')}</div><div class="hint">Anyone can start · ${n < 2 ? 'players can still join after launch' : n + ' players ready'}</div>
    </div>`;
}
let lobbyList = null, listBusy = false;
function lobbyListHTML() {
  if (listBusy) return '<div class="hint">Looking for lobbies…</div>';
  if (!lobbyList) return '<div class="hint">Click REFRESH to find open lobbies</div>';
  if (!lobbyList.length) return '<div class="hint">Click QUICK PLAY to join a lobby</div>';
  return lobbyList.map((l) => `<div class="lobbyrow"><span class="code">${esc(l.code)}</span><span>${esc(l.hostName || 'Someone')}'s lobby</span><span>${l.players}/${l.max}${l.inMatch ? ' · IN MATCH' : ''}</span>${l.full ? '<span class="status">FULL</span>' : `<button type="button" data-join="${esc(l.code)}">JOIN</button>`}</div>`).join('');
}
...
function wireOnline() {
  ...
  if (q('joinBtn')) { q('joinBtn').addEventListener('click', () => { const c = q('codeBox').value.trim().toUpperCase(); if (!c) { setStatus('ENTER THE LOBBY CODE FROM YOUR FRIEND'); return; } lockButtons(box); joinLobby(c); }); q('codeBox').addEventListener('keydown', (e) => { if (e.key === 'Enter') q('joinBtn').click(); }); }
...
function showPause() {
  if (online()) {
    hud.showScreen(`<h1>MENU</h1><h2>FREE-FOR-ALL · LOBBY ${String(net.aliasCode || net.code || '').replace(/-\d+$/, '')}</h2><div class="scoreboard">${sortedScores().map(([id, s]) => `<div class="${id === net.id ? 'me' : ''}"><span>${esc(s.name)}</span><span>${s.kills} KILLS · ${s.deaths} DEATHS</span></div>`).join('')}</div>${CONTROLS_HTML}${settingsHTML()}<div class="online" id="online"><div class="row"><button type="button" class="alt" id="leaveBtn">LEAVE MATCH</button></div></div><div class="go">Click anywhere or press ${hud.key('confirm')} to resume</div>`);
    wireSettings(); wireOnline(); return;
  }
  hud.showScreen(`<h1>PAUSED</h1><h2>WAVE ${game.wave} · SCORE ${game.score}</h2>${CONTROLS_HTML}${settingsHTML()}${menuBtnHTML()}<div class="go">Click anywhere or press ${hud.key('confirm')} to continue</div>`);
  wireSettings(); wireMenuBtn();
}
function showClickToPlay() { hud.showScreen(`<h1>MATCH START</h1><h2>FREE-FOR-ALL · FIRST TO ${FFA_TARGET} KILLS</h2><div class="go">Click anywhere or press ${hud.key('confirm')} to play</div>`); }
function showDead() {
  hud.setGameplayVisible(false); const nb = game.score > best; if (nb) { best = game.score; localStorage.setItem('doodle_best', String(best)); }
  hud.showScreen(`<h1>ERASED</h1><div class="stats">YOU SURVIVED <b>${game.wave}</b> WAVES · <b>${game.kills}</b> KILLS · SCORE <b>${game.score}</b>${nb ? ' · <b>NEW RECORD</b>' : ` · BEST ${best}`}</div>${checkpointHTML()}${menuBtnHTML()}<div class="go">Click or press ${hud.key('confirm')} to try again</div>`);
  ...
  if (playing) {
    ...
    if (st === 'dying') {
      ...
      if (online()) {
        ...
        if (left > 0) { if (left !== before || game.deathT <= dt) hud.message(String(left), 'RETURNING TO THE FIGHT', 1.1); }
          ...
          game.promptT -= dt; if (game.promptT <= 0) { game.promptT = 1.4; hud.message('READY', `Press ${hud.key('confirm')} · any key or click to respawn`, 1.5); }
